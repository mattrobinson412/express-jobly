...
class Company {
  ...
  static async findAll(searchFilters = {}) {
    const companiesRes = await db.query(
      ...
      // For each search term, add to expressions and queryVals so
      // correct SQL is created
      ...
      if (expressions.length > 0) {
        companiesRes += " WHERE " + expressions.join(" AND ");
      }
  
      // Finalize and return results
  
      companiesRes += " ORDER BY name";
      const finalRes = await db.query(companiesRes, queryVals);
